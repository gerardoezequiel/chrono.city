    ...
    <div className="relative shrink-0">
      {showSearch ? (
        <div className="px-6 py-3 bg-neutral-50 border-b border-neutral-200">
          <div className="relative">
            ...
            <input
              ...
              autoFocus={!origin}
              className="w-full pl-9 pr-8 py-2 bg-white text-neutral-900 placeholder-neutral-400 border-2 border-neutral-200 focus:border-neutral-900 focus:outline-none font-body text-sm"
            />
            {origin && (
              <button
              ...
              {results.map((result, i) => (
                <li key={`${result.lngLat.lng}-${result.lngLat.lat}-${i}`}>
                  <button
                    ...
                    className="w-full text-left px-4 py-2.5 text-sm text-neutral-700 hover:bg-neutral-900 hover:text-white transition-colors cursor-pointer font-body"
                  >
        ...
        <button
          ...
          className="w-full text-left group cursor-pointer border-b-2 border-neutral-200 hover:border-neutral-900 transition-colors"
        >
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0 flex-1">
                  ...
                  {reverseResult.city && reverseResult.city !== reverseResult.street && (
                    <p className="font-mono text-[11px] text-neutral-500 uppercase tracking-wider mt-0.5 truncate">
                      {reverseResult.city}
                    </p>
                  )}
                </div>
                {/* Search icon */}
                <svg className="w-3.5 h-3.5 text-neutral-300 group-hover:text-neutral-900 transition-colors shrink-0 mt-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
            ) : (
              <div className="flex items-center justify-between">
                <p className="font-mono text-[11px] text-neutral-400 animate-pulse uppercase tracking-wider">Locating...</p>
                <svg className="w-3.5 h-3.5 text-neutral-300 group-hover:text-neutral-900 transition-colors shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" />
                </svg>
              </div>
            )}
            {/* Coordinates — red accent, always visible when origin set */}
            {origin && (
              <p className="font-mono text-[9px] text-red-600/50 tabular-nums mt-1.5 tracking-wider">
